...
4. [Обучение модели](#обучение-модели)
   ...
   - [Мониторинг обучения](#мониторинг-обучения-mlflow)
5. [Инференс модели](#инференс-модели)
   - [Infer](#infer)
6. [Архитектура проекта](#архитектура-проекта)
7. [Технические детали](#технические-детали)
   - [Используемые технологии](#используемые-технологии)
8. [Дополнительно (разработка)](#дополнительно-разработка)
   - [pytest](#pytest)
...
### Мониторинг обучения (MLflow)

```bash
mlflow ui --port 8080
```

Перейдите на http://127.0.0.1:8080 для просмотра экспериментов и метрик.

Логируемые метрики:

- train_loss — функция потерь на тренировочном датасете
- val_loss — функция потерь на валидации
- val_f1 — макро-F1 по предсказанным дискурс-меткам
...
│   ├── infer
│   │   └── checkpoint_path.yaml    # путь к чекпоинту модели
│   ├── logger                      # настройка логгеров
│   │   ├── csv_logger.yaml
│   │   └── mlflow_logger.yaml
│   ├── model
...
│   ├── python_logging
│   │   └── setup.yaml              # конфигурация питоновского логгера
│   ├── train
